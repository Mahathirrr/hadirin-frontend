...
export const events: CalendarEvent[] = eventsData.map((event) => {
  ...
  const dayAndTime = (event as { date: string }).date.split('T')
  const day = parseInt(dayAndTime[0], 10)
  ...
  return {
    ...(event as Omit<CalendarEvent, 'date'>),
    date: eventDate,
    type: (event as { type: string }).type as CalendarEvent['type'],
  }
})
...
export const eventDates = eventDatesData.map((item) => {
  ...
  const day = parseInt(item.date.split('T')[0], 10)
...
// Calendars data
export const calendars: Calendar[] = calendarsData as unknown as Calendar[]
